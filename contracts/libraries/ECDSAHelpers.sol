...
library ECDSAHelpers {
  ...
  function split(bytes memory signature)
    private
    pure
    returns (
      bytes32 r,
  ...
  function recover(bytes32 messageHash, bytes memory signature) internal pure returns (address signer) {
